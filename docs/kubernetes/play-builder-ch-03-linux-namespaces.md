...
네임스페이스는 커널이 제공하는 격리 기능이며, **kubelet**·**containerd**·애플리케이션 프로세스는 사용자 공간에서 **시스템 콜**을 통해 커널에 요청한다. 아래 그림은 그 관계를 단순화한 것이다. **kubelet**, **containerd**, 웹 서버(예: nginx) 등은 모두 사용자 공간에서 돌고, 하드웨어와 직접 맞닿는 일은 **시스템 콜 인터페이스**를 넘어 **커널 스페이스**의 스케줄러·메모리 관리·VFS·**cgroups**·네트워크 스택·**Namespaces**가 처리한다. 컨테이너가 “가벼운 격리”를 얻는 이유도 결국 이 커널 기능들을 쓰기 때문이다.

![유저 스페이스와 커널 스페이스, 시스템 콜 인터페이스](./assets/play-builder-ch-02-user-kernel-space.png)
